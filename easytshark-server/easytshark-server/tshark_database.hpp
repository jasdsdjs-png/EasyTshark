...
public:
    ...
    bool storePackets(std::vector<std::shared_ptr<Packet>>& packets) {
        ...
        if (!transaction.begin()) {
            LOG_F(ERROR, "Failed to begin packet transaction: %s", sqlite3_errmsg(db));
            return false;
        }
        ...
        StatementGuard stmt(db, insertSQL);
        if (!stmt.valid()) {
        ...
        for (const auto& packet : packets) {
            sqlite3_bind_int(stmt.get(), 1, packet->frame_number);
            sqlite3_bind_double(stmt.get(), 2, packet->time);
            sqlite3_bind_int(stmt.get(), 3, packet->cap_len);
            sqlite3_bind_int(stmt.get(), 4, packet->len);
            bindText(stmt.get(), 5, packet->src_mac);
            bindText(stmt.get(), 6, packet->dst_mac);
            bindText(stmt.get(), 7, packet->src_ip);
            bindText(stmt.get(), 8, packet->src_location);
            sqlite3_bind_int(stmt.get(), 9, packet->src_port);
            bindText(stmt.get(), 10, packet->dst_ip);
            bindText(stmt.get(), 11, packet->dst_location);
            sqlite3_bind_int(stmt.get(), 12, packet->dst_port);
            bindText(stmt.get(), 13, packet->protocol);
            bindText(stmt.get(), 14, packet->info);
            sqlite3_bind_int(stmt.get(), 15, packet->file_offset);
            sqlite3_bind_int(stmt.get(), 16, packet->belong_session_id);
        ...
        if (!transaction.commit()) {
            LOG_F(ERROR, "Failed to commit packet transaction: %s", sqlite3_errmsg(db));
            return false;
        }
        return true;
    }
    ...
    void storeAndUpdateSessions(std::unordered_set<std::shared_ptr<Session>>& sessions) {
        ...
        TransactionGuard transaction(db);
        if (!transaction.begin()) {
            throw std::runtime_error("Failed to begin session transaction");
        }
        ...
        StatementGuard stmt(db, upsertSQL);
        if (!stmt.valid()) {
            throw std::runtime_error("Failed to prepare UPSERT statement");
        }

        for (const auto& session : sessions) {
            sqlite3_bind_int(stmt.get(), 1, session->session_id);
            bindText(stmt.get(), 2, session->ip1);
            bindText(stmt.get(), 3, session->ip1_location);
            sqlite3_bind_int(stmt.get(), 4, session->ip1_port);
            bindText(stmt.get(), 5, session->ip2);
            bindText(stmt.get(), 6, session->ip2_location);
            sqlite3_bind_int(stmt.get(), 7, session->ip2_port);
            bindText(stmt.get(), 8, session->trans_proto);
            bindText(stmt.get(), 9, session->app_proto);
            sqlite3_bind_double(stmt.get(), 10, session->start_time);
            sqlite3_bind_double(stmt.get(), 11, session->end_time);
            sqlite3_bind_int(stmt.get(), 12, session->ip1_send_packets_count);
            sqlite3_bind_int(stmt.get(), 13, session->ip1_send_bytes_count);
            sqlite3_bind_int(stmt.get(), 14, session->ip2_send_packets_count);
            sqlite3_bind_int(stmt.get(), 15, session->ip2_send_bytes_count);
            sqlite3_bind_int(stmt.get(), 16, session->packet_count);
            sqlite3_bind_int(stmt.get(), 17, session->total_bytes);
        ...
        if (!transaction.commit()) {
            throw std::runtime_error("Failed to commit session transaction");
        }
    }
    // 从数据库查询数据包分页数据
...
private:
    ...
    public:
        StatementGuard(sqlite3* db, const std::string& sql) : db_(db) {
